...
const Conversations = (props) => {
  ...
  const handleRecipient = (recipients) => {
    for (let i = 0; i < recipients.length; i++) {
      if (recipients[i].username !== cookie.get("username")) {
        return recipients[i];
      }
    }
    return null;
  ...
  useEffect(() => {
    ...
    socket.on(cookie.get("user_id"), (data) => setNewConversation(data));
      ...
      {conversations && (
        <React.Fragment>
          {conversations.map((c) => (
            ...
            >
              <ListItemAvatar>
                <Avatar>{handleRecipient(c.recipientObj).name[0]}</Avatar>
              </ListItemAvatar>
              <ListItemText
